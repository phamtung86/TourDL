...
async function fetchTourDetail(tourID) {
    try {
        const response = await axios.get(`http://localhost:3124/api/v1/tours/${tourID}`)
        if (response.status == 200) {
            ...
            document.querySelector('.overview-content .overview-i1 .overview-item .dtq').textContent = tour.tourDetail.sight_seeing;
            document.querySelector('.overview-content .overview-i1 .overview-item .at').textContent = tour.tourDetail.cuisine;
            document.querySelector('.overview-content .overview-i1 .overview-item .dt').textContent = tour.tourDetail.suitable_people;

            document.querySelector('.overview-content .overview-i2 .overview-item .tg').textContent = tour.tourDetail.time_suiable;
            document.querySelector('.overview-content .overview-i2 .overview-item .pt').textContent = tour.tourDetail.transport;
            document.querySelector('.overview-content .overview-i2 .overview-item .km').textContent = tour.tourDetail.sale_description;
            ...
            calendars.forEach(calendar => {
                ...
                const formattedDate = new Date(calendar.start_date).toLocaleDateString();
            ...
            tourDateSelect.addEventListener("change", (event) => {
                ...
                const selectedCalendar = calendars.find(calendar => calendar.id == selectedDateId);
